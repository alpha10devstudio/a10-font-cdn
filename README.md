🎨 A10 Font CDN

A fast, lightweight, and free Bengali Web Font CDN powered by GitHub and jsDelivr.

A10 Font CDN provides a simple way to use Bengali web fonts in websites and web applications without manually hosting or downloading font files.

---

🚀 Quick Start

Add the following "<link>" tag inside the "<head>" section of your HTML file:

<link
  rel="stylesheet"
  href="https://cdn.jsdelivr.net/gh/alpha10devstudio/a10-font-cdn@v1.0.0/style.css"
>

Once added, all available fonts can be used through CSS.

---

🔤 Available Fonts

Font Name| CSS "font-family"
এস কে বিশাল| "'SK Bishal'"
আলিনুর নক্ষত্র| "'Alinur Nokshotro'"
আলিনুর নবীন| "'Alinur Nobin'"
মানোয়ার নীহার| "'Manowar Nihar'"
সিরাজী সানজার| "'Siraji Sanjar'"
আলিনুর তৎসম| "'Alinur Totsomo'"
আলিনুর উল্লাস| "'Alinur Ullash'"
শহীদ আবু সাঈদ| "'Shahid Abu Sayeed'"

---

💻 Usage

After loading the CDN stylesheet, use any available font with the corresponding CSS "font-family".

Example

body {
  font-family: 'SK Bishal', sans-serif;
}

h1 {
  font-family: 'Shahid Abu Sayeed', sans-serif;
}

p {
  font-family: 'Alinur Nokshotro', sans-serif;
}

---

🌐 HTML Example

<!DOCTYPE html>
<html lang="bn">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">

  <title>A10 Font CDN</title>

  <link
    rel="stylesheet"
    href="https://cdn.jsdelivr.net/gh/alpha10devstudio/a10-font-cdn@v1.0.0/style.css"
  >

  <style>
    body {
      font-family: 'SK Bishal', sans-serif;
    }

    h1 {
      font-family: 'Shahid Abu Sayeed', sans-serif;
    }
  </style>
</head>

<body>

  <h1>বাংলা ফন্ট পরীক্ষা</h1>

  <p>
    A10 Font CDN ব্যবহার করে সহজেই ওয়েবসাইটে বাংলা ফন্ট ব্যবহার করুন।
  </p>

</body>
</html>

---

📌 Version

Current Version: "v1.0.0"

Stable CDN

https://cdn.jsdelivr.net/gh/alpha10devstudio/a10-font-cdn@v1.0.0/style.css

Using a specific version ensures that your project continues to use the same font release even when newer versions are published.

---

⚡ CDN

A10 Font CDN uses jsDelivr for fast and reliable global delivery.

CDN Base URL

https://cdn.jsdelivr.net/gh/alpha10devstudio/a10-font-cdn@v1.0.0/

Stylesheet

https://cdn.jsdelivr.net/gh/alpha10devstudio/a10-font-cdn@v1.0.0/style.css

---

🔴 Fonts Testing / Live Preview

Preview and test the available Bengali fonts directly in your browser:

"Live Font Preview" (https://htmlpreview.github.io/?https://github.com/alpha10devstudio/a10-font-cdn/blob/main/demo.html)

---

✨ Features

- ⚡ Fast CDN delivery
- 🪶 Lightweight
- 🇧🇩 Bengali web font support
- 🌐 GitHub + jsDelivr powered
- 📱 Mobile-friendly
- 🔗 Easy CSS integration
- 📦 Version-specific CDN support
- 🆓 Free CDN delivery

---

📁 Project Structure

a10-font-cdn/
│
├── fonts/
│   ├── SK-Bishal/
│   ├── Alinur-Nokshotro/
│   ├── Alinur-Nobin/
│   ├── Manowar-Nihar/
│   ├── Siraji-Sanjar/
│   ├── Alinur-Totsomo/
│   ├── Alinur-Ullash/
│   └── Shahid-Abu-Sayeed/
│
├── style.css
│
├── demo.html
│
└── README.md

---

📜 License & Credits

The Bengali fonts included in this repository are sourced from Lipighor.

A10 Font CDN does not claim ownership of these fonts. The fonts remain subject to the copyrights, licenses, and usage terms of their respective creators and source.

This repository provides CDN hosting and web delivery through GitHub and jsDelivr for convenient use in websites and web applications.

Please review and respect the original font licenses and the terms provided by Lipighor before using these fonts in personal, commercial, or redistributed projects.

Font Source

Lipighor — Bengali Font Source

---

👨‍💻 Maintained By

Alpha 10 Dev Studio

«BUILD SMARTER. CREATE FASTER.»

GitHub: "alpha10devstudio" (https://github.com/alpha10devstudio)

---

⭐ Support

If you find A10 Font CDN useful, consider giving the repository a ⭐ on GitHub.
